...
#include "ros_comm_interfaces/NavGraphSetBoundingBox.hpp"
...
void NavGraphSetBoundingBox::create_request(const std::string &env_name,
  ...
  request_clients_[env_name][service_name] =
      this->create_client<expertino_msgs::srv::NavGraphInterfaceSetBoundingBox>(
          service_name);
  requests_[env_name][service_name] = std::make_shared<
      expertino_msgs::srv::NavGraphInterfaceSetBoundingBox::Request>();
}

void NavGraphSetBoundingBox::set_field_request(const std::string &env_name,
                                               const std::string &service_name,
...
void NavGraphSetBoundingBox::set_array_request(const std::string &env_name,
                                               const std::string &service_name,
                                               const std::string &field_name,
                                               CLIPS::Values values) {
  RCLCPP_INFO(rclcpp::get_logger(clips_feature_name),
...
void NavGraphSetBoundingBox::service_callback(
  ...
  while (old_fact) {
    if (old_fact->get_template()->name() ==
            "ros-nav_graph_set_bounding_box-response" &&
  ...
  CLIPS::Template::pointer fact_template =
      envs_[env_name]->get_template("ros-nav_graph_set_bounding_box-message");
