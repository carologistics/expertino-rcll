...
class ExpertinoEnv(CXRLGym):
    def __init__(self, node: Node, mode: str):
        ...
        self.start_refbox_server = ActionServer(
            self.node,
            StartRefbox,
            "cx_rl_node/start_refbox",
            self.start_refbox_callback,
        )
        self.stop_refbox_server = ActionServer(
            self.node, StopRefbox, "cx_rl_node/stop_refbox", self.stop_refbox_callback
        )
    ...
    def step(self, action):
        ...
        with open(self.log_file, "a+") as f:
            f.write(
                f"Action: {self.action_dict[action]} | Reward: {reward} | Done: {done} | Info: {info}\n"
            )
