...
(defrule plan-start-status-checks
  ?gr-f <- (cx-pddl-interfaces-plan-temporal-goal-response (server ?server) (client-goal-handle-ptr ?cgh-ptr))
  ?pc-f <- (pddl-planner-call (status PENDING))
  =>
  (bind ?status (cx-pddl-interfaces-plan-temporal-client-goal-handle-get-status ?cgh-ptr))
  (bind ?uuid (cx-pddl-interfaces-plan-temporal-client-goal-handle-get-goal-id ?cgh-ptr))
...
(defrule plan-update-plan-status
  ...
  =>
  (bind ?new-status (cx-pddl-interfaces-plan-temporal-client-goal-handle-get-status ?cgh-ptr))
...
(defrule plan-get-result
  ...
  ?wr-f <- (cx-pddl-interfaces-plan-temporal-wrapped-result (server "/pddl_manager/temp_plan") (goal-id ?goal-id) (code SUCCEEDED) (result-ptr ?res-ptr))
  =>
  ...
  (if ?plan-found then
    ...
    (foreach ?action ?plan
      (bind ?instance (sym-cat (cx-pddl-interfaces-timed-plan-action-get-field ?action "pddl_instance")))
      ...
      (bind ?equiv_class (cx-pddl-interfaces-timed-plan-action-get-field ?action "equiv_class"))
  ...
  (cx-pddl-interfaces-plan-temporal-client-goal-handle-destroy ?cgh-ptr)
  (retract ?pc-f)
  (retract ?wr-f)
)
