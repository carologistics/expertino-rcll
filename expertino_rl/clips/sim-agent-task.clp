(defrule agent-task-list-create
  ?ex <- (executor (id ?ex-id) (state INIT) (worker ?robot) (action-id ?action-id))
...
(defrule agent-task-list-delete-active
  ...
  (executor (id ?id) (state ACCEPTED) (action-id ?action-id) (worker ?robot))
...
(defrule agent-task-list-mark-done
  ...
  ?ex <- (executor (id ?ex-id) (state ACCEPTED) (action-id ?action-id))
...
(defrule agent-task-outcome-failed
  ...
  ?ex <- (executor (id ?ex-id) (state ACCEPTED) (action-id ?action-id) (worker ?robot))
  ?cur-task-seq <- (current-rcll-agent-task-id (task-id ?seq) (robot ?robot))
