...
(defrule process-orders
    ?f <- (order-processed $?orders)
    =>
    (foreach ?order-id $?orders
        (printout t "Processing Order ID: " ?order-id crlf)
    )
)
...
(defrule add-order-to-problem
  ...
  (foreach ?i length$(order-processed (id ?order-id))
    (bind ?instance (sym-cat ?instance-str))
