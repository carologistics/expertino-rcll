...
(defrule add-ring-specs-to-problem
  ...
  (delayed-do-for-all-facts ((?ring-spec ring-spec)) TRUE
    ...
    (foreach ?i (create$ 1 2 3)
      (assert (pddl-numeric-fluent-change (instance ?instance) (name price)
...
(defrule add-order-to-problem
  ...
  (assert (pddl-object-change (instance ?instance) (name ?wp) (type product)))
  (assert (pddl-fluent-change (instance ?instance) (name spawnable) (params ?wp)))
  (bind ?curr-step (wp-part-to-pddl ?base-col))
  (assert (pddl-fluent-change (instance ?instance) (name step) (params ?wp ?curr-step)))
  ...
  (while (neq ?ring-steps (create$))
    ...
    (assert (pddl-fluent-change (instance ?instance) (name next-step) (params ?wp ?curr-step ?next-step)))
    (bind ?curr-step ?next-step)
    (bind ?ring-num (+ ?ring-num 1))
  )
  (bind ?next-step (wp-part-to-pddl ?cap-col))
  (assert (pddl-fluent-change (instance ?instance) (name next-step) (params ?wp ?curr-step ?next-step)))
  (assert (pddl-fluent-change (instance ?instance) (name next-step) (params ?wp ?next-step deliver)))
  (assert (pddl-fluent-change (instance ?instance) (name next-step) (params ?wp deliver done)))
