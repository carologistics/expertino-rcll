...
(deftemplate plan-generated-time
  (slot time))
...
(deffunction get-current-time ()
  (return (get-current-system-time))  
)

(deftemplate delayed-planner-update
  (slot delay-count)  ; Count down the delay cycles.
)
...
(defrule set-plan-generated-time
  (startup-completed)
  (not (plan-generated-time))
  =>
  (bind ?current-time (get-current-time)) ; You can replace get-current-time with an appropriate function to get the current time in your system.
  (assert (plan-generated-time (time ?current-time)))
  (printout t "Plan generated at time: " ?current-time crlf)
)

(defrule trigger-planner-update-after-delay
  (plan-generated-time (time ?start-time))   
  (not (request-planner-update))              
  =>
  (bind ?current-time (get-current-time))   
  (bind ?delay 10)                           
  (bind ?time-difference (- ?current-time ?start-time)) 
  (if (>= ?time-difference ?delay) then
      (progn
        (printout t "Time delay has passed. Triggering planner update..." crlf)
        (assert (request-planner-update))   ; Trigger the planner update.
      )
  )
)

(defrule set-delayed-planner-update
  (plan-generated)  ; This is the fact that indicates the plan was generated.
  =>
  (assert (delayed-planner-update (delay-count 5)))  ; Set a delay of 5 cycles (or any other number of cycles).
  (printout t "Planner update delayed by 5 cycles." crlf)
)


(defrule countdown-delayed-planner-update
  (delayed-planner-update (delay-count ?count))
  (not (request-planner-update))  ; Ensure the planner update hasn't been triggered yet.
  =>
  (if (> ?count 0) then
      (progn
        (bind ?new-count (- ?count 1))
        (modify (delayed-planner-update (delay-count ?new-count)))  ; Decrement the delay count.
      )
  )
  (if (eq ?count 0) then
      (progn
        (printout t "Delay period passed. Triggering planner update..." crlf)
        (assert (request-planner-update))   ; Trigger the planner update after the delay.
      )
  )
)
...
(defrule goal-updated-start-plan
  ...
  (assert (request-planner-update))
)


(defrule trigger-planner-update
  ?set-f <- (pddl-set-goals (instance ?instance) (state DONE))
  (pddl-manager (node ?node))
  (pddl-instance (name ?instance) (busy-with FALSE) (state LOADED))
  (planning-filter (action-names $?an))
  (expertino-msgs-plan-temporal-client (server ?server&:(eq ?server (str-cat ?node "/temp_plan"))))
  (planned-for-main) 
  (request-planner-update)
  =>
  (printout t "Triggering planner update..." crlf)
  (bind ?goal (expertino-msgs-plan-temporal-goal-create))
  (assert (pddl-planner-call (context test-plan) (goal ?goal)))
  (expertino-msgs-plan-temporal-goal-set-field ?goal "pddl_instance" ?instance)
  (expertino-msgs-plan-temporal-goal-set-field ?goal "action_names" ?an)
