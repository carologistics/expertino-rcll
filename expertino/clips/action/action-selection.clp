...
(defrule agenda-action-sat-check-start
  ...
  =>
  (assert (pddl-action-condition (instance ?instance) (action ?action-id) (plan ?plan-id) (state PENDING) (instance-update ?last-update-time)))
)
...
(defrule agenda-action-sat-check-reset
  ...
  ?precon <- (pddl-action-condition (action ?action-id) (state CONDITION-UNSAT|CONDITION-SAT|PENDING) (instance-update ?update-time))
  (worker (id ?worker) (state IDLE) (type ?worker-type))
  (pddl-plan (id ?plan-id) (instance ?instance))
...
(defrule agenda-action-sat-check-apply-results
  ...
  (pddl-action-condition (action ?action-id) (state ?state&CONDITION-UNSAT|CONDITION-SAT))
  =>
