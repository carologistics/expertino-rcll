...
(defrule agenda-select-first-plan
  ?plan <- (pddl-plan (id ?plan-id) (state SUCCESS))
...
"When all parallel actions at a particular time are done, move the timeline forward."
  (agenda-action-item (plan ?plan-id) (action ?id) (execution-state COMPLETED))
  (pddl-action (id ?id) (plan ?plan-id) (planned-start-time ?st) (planned-duration ?d))
  ?a <- (agenda (plan ?plan-id) (current-time ?st))
  (not (and
          (pddl-action (id ?o-id&:(neq ?o-id ?id)) (plan ?plan-id) (planned-start-time ?st) (planned-duration ?d))
          (or (agenda-action-item (plan ?plan-id) (action ?o-id) (execution-state ~COMPLETED))
              (not (agenda-action-item (plan ?plan-id) (action ?o-id)))
          )
  ))
  =>
  (modify ?a (current-time (+ ?st ?d))) 
  (printout green "Agenda moved ahead to time " (+ ?d ?st) crlf)
)

(defrule agenda-add-action
  (agenda (plan ?plan-id) (current-time ?st) (state ACTIVE))
  ?pa <- (pddl-action (id ?action-id) (name ?action-name) (plan ?plan-id) (planned-start-time ?t))
  ?a <- (agenda (plan ?plan-id) (current-time ?st&:(< (- ?t ?st) 1)) (state ACTIVE))
  (not (agenda-action-item (plan ?plan-id) (action ?action-id)))
  ;(not (pddl-action (id ?o-id&:(neq ?action-id ?o-id)) (plan ?plan-id) 
  ;       (planned-start-time ?o-st&:(< ?o-st ?st)) (planned-duration ?o-d&:(< (+ ?o-st ?o-d) ?st)))
  ;)
  ...
  (printout yellow "Added action " ?action-id " to agenda at current time " ?t crlf)
  (modify ?a (current-time ?t))
)
...
;  ?precon <- (pddl-action-condition (plan ?plan-id) (action ?action-id) (state CONDITION-SAT) (context AGENDA-LOOKAHEAD))
;  (not (agenda-action-item (plan ?plan-id) (action ?action-id)))
;  (confval (path "/pddl/actions/robot") (list-value $?robot-actions))
;  (confval (path "/pddl/actions/refbox") (list-value $?refbox-actions))
;  (confval (path "/pddl/actions/agent") (list-value $?agent-actions))
...
;    (not (pddl-action-condition (plan ?plan-id) (action ?action-id) (context AGENDA-LOOKAHEAD)))
;    (not (agenda-action-item (plan ?plan-id) (action ?action-id)))
;    (pddl-plan (id ?plan-id) (instance ?instance))
;    (pddl-instance-update (instance ?instance) (last-updated ?last-update-time))
;    =>
;    (assert (pddl-action-condition (instance ?instance) (plan ?plan-id) (action ?action-id) (state PENDING) (context AGENDA-LOOKAHEAD) (instance-update ?last-update-time)))
...
;  (agenda (plan ?plan-id) (class-selection ?ordering-class-selected) (class-relaxation ?relaxation) (state ACTIVE))
;  (pddl-action (id ?action-id) (plan ?plan-id) (plan-order-class ?plan-order-class&:(>= ?ordering-class-selected  (- ?plan-order-class ?relaxation))))
;  ?precon <- (pddl-action-condition (plan ?plan-id) (action ?action-id) (context AGENDA-LOOKAHEAD) (state CONDITION-UNSAT|CONDITION-SAT|PENDING) (instance-update ?update-time))
...
(defrule agenda-complete
  ...
  (not (and  (pddl-action (id ?action-id) (plan ?plan-id))
      (not (agenda-action-item (plan ?plan-id) (action ?action-id)))
  ))
  ...
  (modify ?agenda (state COMPLETED))
) 

(defrule agenda-cleanup
  (declare (salience ?*SALIENCE-LOW*))
  ?agenda <- (agenda (plan ?plan-id) (state COMPLETED))
