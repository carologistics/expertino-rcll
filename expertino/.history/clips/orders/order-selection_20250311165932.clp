...
(defrule wm-updated-plan
  (startup-completed)
  ?planned <- (planned-for-main)
  (pddl-planner-call (context test-plan) (goal ?goal) (state SUCCESS))
  (pddl-instance (name ?instance) (state LOADED))
  ?worldmodel <- (worldmodel-updated)
  (order (id ?order-id) (state ACTIVE))
  =>
  (printout green "Updating world model with new plan execution..." crlf)
  ; Apply the effects of the executed action
  (delayed-do-for-all-facts ((?fluent pending-pddl-fluent)) (eq ?fluent:instance ?instance)
    (assert (applied-action-effect (instance ?instance) (fluent ?fluent:name) (params ?fluent:params)))
    (retract ?fluent)
  )
  ; Update order state if necessary
  (modify ?planned (state EXECUTED))
  (modify ?worldmodel (state PROCESSED))
  (assert (planning-cycle-completed))
