...
(defrule first-action-executed
  ?exec <- (pddl-action-executed (instance ?instance) (action ?action) (state DONE))
  (not (pddl-action-executed (instance ?instance) (state IN_PROGRESS))) 
  (not (first-action-detected))
  =>
  (printout green "First action executed. Generating updated plan with one random order..." crlf)
  (assert (first-action-detected))
  (assert (generate-updated-plan ?instance))
)
