...
(defrule planning-completed-add-next-order
  ?plan-done <- (pddl-planner-call (context test-plan) (state SUCCESS))
  (not (added-all-orders))
  ...
  (assert (append-new-plan))  ; This fact must be defined somewhere before use
  (assert (pddl-set-goals (instance ?instance)))  ; Re-trigger goal setting
  (retract ?plan-done)  ; Corrected retract statement
)
...
(defrule append-plan-to-existing
  ...
  (retract (append-new-plan)) ; Only retract if it's no longer needed
)
