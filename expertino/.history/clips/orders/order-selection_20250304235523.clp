...
(defrule goal-updated-start-plan
  ...
  (assert (planning-done))
)

(defrule trigger-next-order-planning
  (planning-done)
  ?order-f <- (order (id ?order-id) (state OPEN))
  (not (order-scheduled (id ?order-id)))  ;; Ensure it's not already scheduled
  =>
  (printout t "Triggering the next order scheduling for order " ?order-id crlf)
  (assert (insert-order (T-last-end ?last-end) (T-order-window ?window)))
  (assert (order-scheduled (id ?order-id))) ;; Mark it as scheduled
  (retract ?order-f) ;; Change order state
  (assert (order (id ?order-id) (state ACTIVE)))
  (retract (planning-done)) ;; Reset flag for next iteration
)
