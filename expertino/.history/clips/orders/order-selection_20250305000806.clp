...
(defrule trigger-replanning
  (trigger-replanning)  ; Check if the flag to trigger replanning is set
  =>
  (printout t "Replanning triggered due to new order." crlf)
  (assert (pddl-planner-call (context test-plan) (goal ?goal)))  ; Call the PDDL planner
  (retract (trigger-replanning))  ; Clear the flag to prevent retriggering
)
