...
(defrule add-order-to-problem
  ...
  ;(assert (order-processed (id ?order-id)))
   ; Trigger replanning without hardcoding steps
  (assert (replan-required))
  
  ; Update the order's state to PROCESSED after scheduling
  (modify ?order-f (state PROCESSED))
  (printout t "New order added and replanning triggered." crlf))
...
(defrule trigger-replanning
  (replan-required)
  =>
  (printout t "Replanning required, starting planning process..." crlf)
  
  ; Trigger the planner to start the process
  (assert (pddl-planner-call (context test-plan) (goal ?goal)))
  
  ; Mark that planning has been initiated
  (assert (planned-for-main))
  
  ; Clear the "replan-required" flag after the planning process is triggered
  (retract (replan-required))
  
  (printout t "Replanning initiated. The new order will be considered in the updated plan." crlf))
