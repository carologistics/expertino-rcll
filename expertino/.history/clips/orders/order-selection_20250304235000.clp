...
(defrule planning-completed-add-next-order
  ?plan-done <- (planning-finished)  ;; Replacing pddl-planner-call (context test-plan) with a dedicated fact
  (not (added-all-orders))
  (order (id ?order-id) (state OPEN))
  =>
  (retract ?plan-done)  ; Remove the old planning completion fact
  (printout green "Planning completed, adding another order..." crlf)
  (assert (added-one-order))
  (assert (order-scheduled (id ?order-id)))
  (assert (insert-order (T-last-end ?T_last_end) (T-order-window ?T_order_window)))
  (printout green "Order " ?order-id " added, updating plan..." crlf)
  (assert (append-new-plan))  ; Ensure new plan is appended to the existing one
  (assert (pddl-set-goals (instance ?instance)))  ; Re-trigger goal setting
  (retract (planned-for-main))  ; Allow re-planning
)

(defrule append-plan-to-existing
  (append-new-plan)
  (pddl-instance (name ?instance) (state LOADED))
  (plan-step (id ?id) (task ?task) (start-time ?start-time) (duration ?duration))
  =>
  (assert (plan-step (id (+ ?id 1)) (task ?task) (start-time (+ ?start-time ?duration)) (duration ?duration)))
  (printout green "Appended new plan step for task: " ?task crlf)
  (retract (append-new-plan))
)
