...
/**
   ...
   \remark User must use #Z3_optimize_inc_ref and #Z3_optimize_dec_ref to manage
   optimize objects. Even if the context was created using #Z3_mk_context
...
/**
   ...
   def_API('Z3_optimize_assert_and_track', VOID, (_in(CONTEXT), _in(OPTIMIZE),
   _in(AST), _in(AST)))
*/
void Z3_API Z3_optimize_assert_and_track(Z3_context c, Z3_optimize o, Z3_ast a,
                                         Z3_ast t);
...
/**
   ...
   \param a - formula
   \param weight - a penalty for violating soft constraint. Negative weights
   convert into rewards. \param id - optional identifier to group soft
   constraints

   \sa Z3_optimize_assert
   \sa Z3_optimize_assert_and_track

   def_API('Z3_optimize_assert_soft', UINT, (_in(CONTEXT), _in(OPTIMIZE),
   _in(AST), _in(STRING), _in(SYMBOL)))
*/
unsigned Z3_API Z3_optimize_assert_soft(Z3_context c, Z3_optimize o, Z3_ast a,
...
/**
   ...
   def_API('Z3_optimize_maximize', UINT, (_in(CONTEXT), _in(OPTIMIZE),
   _in(AST)))
*/
...
/**
   ...
   def_API('Z3_optimize_minimize', UINT, (_in(CONTEXT), _in(OPTIMIZE),
   _in(AST)))
*/
...
/**
   ...
   def_API('Z3_optimize_check', LBOOL, (_in(CONTEXT), _in(OPTIMIZE), _in(UINT),
   _in_array(2, AST)))
*/
...
/**
   ...
   def_API('Z3_optimize_get_reason_unknown', STRING, (_in(CONTEXT),
   _in(OPTIMIZE) ))
*/
...
/**
   ...
   def_API('Z3_optimize_get_unsat_core', AST_VECTOR, (_in(CONTEXT),
   _in(OPTIMIZE)))
 */
...
/**
   ...
   def_API('Z3_optimize_set_params', VOID, (_in(CONTEXT), _in(OPTIMIZE),
   _in(PARAMS)))
*/
...
/**
   ...
   def_API('Z3_optimize_get_param_descrs', PARAM_DESCRS, (_in(CONTEXT),
   _in(OPTIMIZE)))
*/
...
/**
   \brief Retrieve lower bound value or approximation for the i'th optimization
   objective.
   ...
   def_API('Z3_optimize_get_lower', AST, (_in(CONTEXT), _in(OPTIMIZE),
   _in(UINT)))
*/
Z3_ast Z3_API Z3_optimize_get_lower(Z3_context c, Z3_optimize o, unsigned idx);

/**
   \brief Retrieve upper bound value or approximation for the i'th optimization
   objective.
   ...
   def_API('Z3_optimize_get_upper', AST, (_in(CONTEXT), _in(OPTIMIZE),
   _in(UINT)))
*/
...
/**
   \brief Retrieve lower bound value or approximation for the i'th optimization
   objective. The returned vector is of length 3. It always contains numerals.
          The three numerals are coefficients \c a, \c b, \c c and encode the
   result of #Z3_optimize_get_lower \ccode{a * infinity + b + c * epsilon}.
   ...
   def_API('Z3_optimize_get_lower_as_vector', AST_VECTOR, (_in(CONTEXT),
   _in(OPTIMIZE), _in(UINT)))
*/
...
/**
   ...
   def_API('Z3_optimize_get_upper_as_vector', AST_VECTOR, (_in(CONTEXT),
   _in(OPTIMIZE), _in(UINT)))
*/
...
/**
   ...
   def_API('Z3_optimize_from_string', VOID, (_in(CONTEXT), _in(OPTIMIZE),
   _in(STRING)))
*/
...
/**
   ...
   def_API('Z3_optimize_from_file', VOID, (_in(CONTEXT), _in(OPTIMIZE),
   _in(STRING)))
*/
void Z3_API Z3_optimize_from_file(Z3_context c, Z3_optimize o, Z3_string s);

/**
   \brief Return a string containing a description of parameters accepted by
   optimize.
...
/**
   ...
   def_API('Z3_optimize_get_assertions', AST_VECTOR, (_in(CONTEXT),
   _in(OPTIMIZE)))
*/
...
/**
   ...
   as a Pseudo-Boolean (minimization) sum of the form \ccode{(+ (if f1 w1 0) (if
   f2 w2 0) ...)} If the objective function is entered as a maximization
   objective, then return the corresponding minimization objective. In this way
   the resulting objective function is always returned as a minimization
   objective.

   def_API('Z3_optimize_get_objectives', AST_VECTOR, (_in(CONTEXT),
   _in(OPTIMIZE)))
*/
 ...
 */
void Z3_API Z3_optimize_register_model_eh(Z3_context c, Z3_optimize o,
                                          Z3_model m, void *ctx,
