...
/**
    \brief Create a numeral of RoundingMode sort which represents the
...
/**
    \brief Create a numeral of RoundingMode sort which represents the TowardZero
   rounding mode.
...
/**
   ...
   expressions.
    ...
    Note that \c sgn is required to be a bit-vector of size 1. Significand and
   exponent are required to be longer than 1 and 2 respectively. The
   FloatingPoint sort of the resulting expression is automatically determined
   from the bit-vector sizes of the arguments. The exponent is assumed to be in
...
/**
   ...
   a string.
    ...
    def_API('Z3_mk_fpa_numeral_float', AST, (_in(CONTEXT), _in(FLOAT),
   _in(SORT)))
*/
...
/**
   ...
   a string.
    ...
    def_API('Z3_mk_fpa_numeral_double', AST, (_in(CONTEXT), _in(DOUBLE),
   _in(SORT)))
*/
...
/**
    \brief Create a numeral of FloatingPoint sort from a sign bit and two
   integers.
    ...
    def_API('Z3_mk_fpa_numeral_int_uint', AST, (_in(CONTEXT), _in(BOOL),
   _in(INT), _in(UINT), _in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_numeral_int_uint(Z3_context c, bool sgn, signed exp,
                                         unsigned sig, Z3_sort ty);

/**
    \brief Create a numeral of FloatingPoint sort from a sign bit and two 64-bit
   integers.
    ...
    def_API('Z3_mk_fpa_numeral_int64_uint64', AST, (_in(CONTEXT), _in(BOOL),
   _in(INT64), _in(UINT64), _in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_numeral_int64_uint64(Z3_context c, bool sgn,
                                             int64_t exp, uint64_t sig,
                                             Z3_sort ty);
...
/**
    ...
    \c rm must be of RoundingMode sort, \c t1 and \c t2 must have the same
   FloatingPoint sort.
...
/**
    ...
    The nodes \c rm must be of RoundingMode sort, \c t1 and \c t2 must have the
...
/**
    ...
    \c rm must be of RoundingMode sort, \c t1, \c t2, and \c t3 must have the
   same FloatingPoint sort.

    def_API('Z3_mk_fpa_fma', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(AST),_in(AST)))
*/
Z3_ast Z3_API Z3_mk_fpa_fma(Z3_context c, Z3_ast rm, Z3_ast t1, Z3_ast t2,
                            Z3_ast t3);
...
/**
    ...
    def_API('Z3_mk_fpa_round_to_integral', AST,
   (_in(CONTEXT),_in(AST),_in(AST)))
*/
...
/**
    \brief Predicate indicating whether \c t is a subnormal floating-point
   number.
...
/**
    \brief Predicate indicating whether \c t is a floating-point number with
   zero value, i.e., +zero or -zero.
...
/**
     \brief Predicate indicating whether \c t is a floating-point number
   representing +oo or -oo.
...
/**
    \brief Predicate indicating whether \c t is a negative floating-point
   number.
...
/**
    \brief Predicate indicating whether \c t is a positive floating-point
   number.
...
/**
    \brief Conversion of a single IEEE 754-2008 bit-vector into a floating-point
   number.

    Produces a term that represents the conversion of a bit-vector term \c bv to
   a floating-point term of sort \c s.

    \param c logical context
    \param bv a bit-vector term
    \param s floating-point sort

    \c s must be a FloatingPoint sort, \c t must be of bit-vector sort, and the
   bit-vector size of \c bv must be equal to \ccode{ebits+sbits} of \c s. The
...
/**
    \brief Conversion of a FloatingPoint term into another term of different
   FloatingPoint sort.

    Produces a term that represents the conversion of a floating-point term \c t
   to a floating-point term of sort \c s. If necessary, the result will be
   rounded according to rounding mode \c rm.
    ...
    \c s must be a FloatingPoint sort, \c rm must be of RoundingMode sort, \c t
   must be of floating-point sort.

    def_API('Z3_mk_fpa_to_fp_float', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_to_fp_float(Z3_context c, Z3_ast rm, Z3_ast t,
                                    Z3_sort s);

/**
    \brief Conversion of a term of real sort into a term of FloatingPoint sort.

    Produces a term that represents the conversion of term \c t of real sort
   into a floating-point term of sort \c s. If necessary, the result will be
   rounded according to rounding mode \c rm.
   ...
   must be of real sort.
    ...
    def_API('Z3_mk_fpa_to_fp_real', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_to_fp_real(Z3_context c, Z3_ast rm, Z3_ast t,
                                   Z3_sort s);

/**
    \brief Conversion of a 2's complement signed bit-vector term into a term of
   FloatingPoint sort.

    Produces a term that represents the conversion of the bit-vector term \c t
   into a floating-point term of sort \c s. The bit-vector \c t is taken to be
   in signed 2's complement format. If necessary, the result will be rounded
   ...
   must be of bit-vector sort.
    ...
    def_API('Z3_mk_fpa_to_fp_signed', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_to_fp_signed(Z3_context c, Z3_ast rm, Z3_ast t,
                                     Z3_sort s);

/**
    \brief Conversion of a 2's complement unsigned bit-vector term into a term
   of FloatingPoint sort.

    Produces a term that represents the conversion of the bit-vector term \c t
   into a floating-point term of sort \c s. The bit-vector \c t is taken to be
   in unsigned 2's complement format. If necessary, the result will be rounded
   according to rounding mode \c rm.
   ...
   must be of bit-vector sort.
    ...
    def_API('Z3_mk_fpa_to_fp_unsigned', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_to_fp_unsigned(Z3_context c, Z3_ast rm, Z3_ast t,
                                       Z3_sort s);

/**
    \brief Conversion of a floating-point term into an unsigned bit-vector.

    Produces a term that represents the conversion of the floating-point term \c
   t into a bit-vector term of size \c sz in unsigned 2's complement format. If
   necessary, the result will be rounded according to rounding mode \c rm.
...
/**
    ...
    Produces a term that represents the conversion of the floating-point term \c
   t into a bit-vector term of size \c sz in signed 2's complement format. If
   necessary, the result will be rounded according to rounding mode \c rm.
...
/**
    ...
    Produces a term that represents the conversion of the floating-point term \c
   t into a real number. Note that this type of conversion will often result in
...
/**@{*/
/**
    \brief Retrieves the number of bits reserved for the exponent in a
   FloatingPoint sort.
...
/**
    \brief Retrieves the number of bits reserved for the significand in a
   FloatingPoint sort.
...
/**
    \brief Retrieves the sign of a floating-point literal as a bit-vector
   expression.
...
/**
    \brief Retrieves the significand of a floating-point literal as a bit-vector
   expression.
...
/**
    ...
    \returns true if \c t corresponds to a floating point numeral, otherwise
   invokes exception handler or returns false

    Remarks: sets \c sgn to 0 if `t' is positive and to 1 otherwise, except for
    NaN, which is an invalid argument.

    def_API('Z3_fpa_get_numeral_sign', BOOL, (_in(CONTEXT), _in(AST),
   _out(INT)))
*/
...
/**
   ...
   invokes exception handler or returns false
    ...
    Remarks: The significand \c s is always \ccode{0.0 <= s < 2.0}; the
   resulting string is long enough to represent the real significand precisely.

    def_API('Z3_fpa_get_numeral_significand_string', STRING, (_in(CONTEXT),
   _in(AST)))
*/
...
/**
    ...
    def_API('Z3_fpa_get_numeral_significand_uint64', BOOL, (_in(CONTEXT),
   _in(AST), _out(UINT64)))
*/
bool Z3_API Z3_fpa_get_numeral_significand_uint64(Z3_context c, Z3_ast t,
                                                  uint64_t *n);
...
/**
    ...
    \param c logical context
    \param t a floating-point numeral
    \param biased flag to indicate whether the result is in biased
representation \returns true if \c t corresponds to a floating point numeral,
otherwise invokes exception handler or returns false
...
/**
    \brief Return the exponent value of a floating-point numeral as a signed
   64-bit integer
   ...
   otherwise invokes exception handler or returns false
    ...
    def_API('Z3_fpa_get_numeral_exponent_int64', BOOL, (_in(CONTEXT), _in(AST),
   _out(INT64), _in(BOOL)))
*/
bool Z3_API Z3_fpa_get_numeral_exponent_int64(Z3_context c, Z3_ast t,
                                              int64_t *n, bool biased);

/**
    \brief Retrieves the exponent of a floating-point literal as a bit-vector
   expression.
...
/**
    \brief Conversion of a floating-point term into a bit-vector term in IEEE
   754-2008 format.

    \param c logical context
    \param t term of FloatingPoint sort

    \c t must have FloatingPoint sort. The size of the resulting bit-vector is
...
/**
    \brief Conversion of a real-sorted significand and an integer-sorted
   exponent into a term of FloatingPoint sort.
    ...
    \c s must be a FloatingPoint sort, \c rm must be of RoundingMode sort, \c
   exp must be of int sort, \c sig must be of real sort.

    def_API('Z3_mk_fpa_to_fp_int_real', AST,
   (_in(CONTEXT),_in(AST),_in(AST),_in(AST),_in(SORT)))
*/
Z3_ast Z3_API Z3_mk_fpa_to_fp_int_real(Z3_context c, Z3_ast rm, Z3_ast exp,
                                       Z3_ast sig, Z3_sort s);
/**@}*/
/**@}*/
/**@}*/
